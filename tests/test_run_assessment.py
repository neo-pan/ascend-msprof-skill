...
class RunAssessmentTests(unittest.TestCase):
    ...
    def pair(self, record=None):
        ...
        return build_comparison(self.a, self.b)
    ...
    def test_benchmark_only_comparison_and_single_run(self):
        ...
        single = build_candidate_summary(self.b)
    ...
    def test_comparison_and_summary_share_assessments(self):
        result = self.pair()
        summary = build_candidate_summary(self.b, self.a)
    ...
    def test_condition_mismatches_block_and_implementation_changes_do_not(self):
        for field, value in (("stride", [1,128]), ("offset", 1), ("dtype", "float16")):
            with self.subTest(field=field):
                ...
                self.write(self.a)
                p = assess_run(RunEvidence.load_assessment(run), RunEvidence.load_assessment(self.a))["performance_assessment"]
    ...
    def test_invalid_inputs_and_all_reasons_survive(self):
        for name, value in (("bool", True), ("nan", float("nan")), ("inf", float("inf")), ("zero", 0), ("negative", -1)):
            with self.subTest(name=name):
                ...
                p = assess_run(RunEvidence.load_assessment(self.root/name))["performance_assessment"]
                self.assertEqual(p["eligibility"]["status"], "blocked")
    ...
    def test_samples_are_raw_context_only_and_named_tensors_align(self):
        record = copy.deepcopy(self.record)
        record["measurement"]["samples_ms"] = [float("nan"), False, -4]
    ...
    def test_duplicates_conflicts_and_source_tampering(self):
        ...
        self.assertEqual(assess_run(RunEvidence.load_assessment(self.b))["performance_assessment"]["eligibility"]["status"], "blocked")
    ...
    def test_legacy_context_does_not_fill_new_measurement(self):
        ...
        result = build_candidate_summary(self.a)
    ...
    def test_profiler_changes_do_not_affect_performance(self):
        ...
        for name, content in (("summary.json", {"target_identity": [], "evidence_readiness":{"level":"insufficient"},"next_collection_actions":[{"id":"unknown", "required_artifacts": None}]}),
            ...
            self.assertEqual(build_comparison(self.a,self.b)["performance_assessment"],expected)
            path.write_text('{bad')
            self.assertEqual(build_comparison(self.a,self.b)["performance_assessment"],expected)
            path.unlink()
    ...
    def test_assessment_computes_without_file_or_device_access(self):
        ...
        with mock.patch.object(Path,"open",side_effect=AssertionError("unexpected I/O")), mock.patch("subprocess.run", side_effect=AssertionError("unexpected command")):
            self.assertEqual(assess_run(b,a)["performance_assessment"]["eligibility"]["status"],"eligible")
    ...
    def test_tilelang_adapter_links_only_matching_payload(self):
        ...
        self.assertEqual(build_candidate_summary(self.a)["mechanism_assessment"]["benchmark_association"][0]["status"],"linked")
    ...
    def test_unsupported_contract_multicase_and_correctness_binding(self):
        for field, value in (("contract_version", "2.0"), ("workload.case_count", 2), ("correctness.subject_id", "wrong-code"), ("measurement.sample_count", 1.5)):
            with self.subTest(field=field):
                ...
                run = self.root / field
                self.write(run, record)
                self.assertEqual(build_candidate_summary(run)["performance_assessment"]["eligibility"]["status"], "blocked")
    ...
    def test_missing_source_remains_blocked_when_another_import_is_added(self):
        ...
        self.assertEqual(build_candidate_summary(self.a)["performance_assessment"]["eligibility"]["status"], "blocked")
    ...
    def test_harness_adapter_retains_profiler_success_and_separate_versions(self):
        ...
        self.assertEqual(json.loads(artifacts.workflow_metadata_path.read_text())["schema_version"], 3)
        self.assertEqual(build_candidate_summary(self.a)["mechanism_assessment"]["benchmark_association"][0]["status"], "linked")
    ...
    def test_metric_decrease_can_coexist_with_runtime_regression(self):
        ...
        for run, value in ((self.a, 0.9), (self.b, 0.7)):
            summary = {"target_identity": {"status": "match", "expected": {"names": ["kernel"]}},
                       "metric_scope": {"value": "PipeUtilization", "artifact": "logs/command.txt"},
                       "headlines": {"pipe_utilization": {"name": "cube0", "value": value, "field": "aic_mte2_ratio",
                           "field_kind": "utilization_or_ratio", "segment": "op", "metric_scope": "PipeUtilization",
                           "file": "reports/op/OPPROF_001/PipeUtilization.csv", "raw_row": {"block_id": "0", "sub_block_id": "cube0", "aic_mte2_ratio": str(value)}}}}
            provenance = {"cann_version": {"value": "8.3.RC2", "source": {"artifact": "logs/cann_version.cfg", "field": "toolkit_running_version"}},
                          "hardware": {"summary": {"value": "Ascend 910B2"}},
                          "profile_command": {"value": "msprof op --aic-metrics=PipeUtilization"}, "profile_output_segments": {"op": {"kind": "op"}}}
    ...
    def test_legacy_caller_speedup_is_not_published_as_an_assessment(self):
        ...
        evidence = RunEvidence.from_loaded(self.a, {}, profile_context=context)
        report = build_report_from_evidence(evidence)
        self.assertNotIn("| Speedup |", report)
        self.assertNotIn("benchmark.candidate.speedup", report)
        self.assertIsNone(assess_run(evidence)["performance_assessment"]["comparison"]["observation"])
    ...
    def test_single_run_json_is_deterministic_across_hash_seeds(self):
        ...
        program = "import json,sys; from pathlib import Path; from ascend_msprof_skill.summarize_candidate import build_candidate_summary; print(json.dumps(build_candidate_summary(Path(sys.argv[1])),sort_keys=True))"
        outputs = []
        for seed in ("1", "2"):
